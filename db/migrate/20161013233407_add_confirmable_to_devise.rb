class AddConfirmableToDevise < ActiveRecord::Migration[5.0]
  def up
    ...
    # add_column :donors, :unconfirmed_email, :string # Only if using reconfirmable
    add_index :donors, :confirmation_token, unique: true
    # User.reset_column_information # Need for some types of updates, but not for update_all.
    # To avoid a short time window between running the migration and updating all existing
    # donors as confirmed, do the following
    execute("UPDATE donors SET confirmed_at = NOW()")
    # All existing user accounts should be able to log in after this.
    # Remind: Rails using SQLite as default. And SQLite has no such function :NOW.
    # Use :date('now') instead of :NOW when using SQLite.
    # => execute("UPDATE donors SET confirmed_at = date('now')")
